...
#   springboot.vm.network "private_network", ip: "192.168.60.167", nic_type: "virtio"
    springboot.vm.provision "shell", :inline => "sudo echo '192.168.60.167 springboot.local springboot' >> /etc/hosts"

##  Use Main / Update in Vagrant provision command ### $vagrant provision --provision-with shell/main/update

    # Default
    springboot.vm.provision "main", type: "ansible" do |ansible|
      ansible.playbook = "deploy_springboot_DEV.local.yml"
#     ansible.playbook = "deploy_springboot_DEV.local.yml"
#     ansible.playbook = "deploy_springbootTestRH7.yml"
      ansible.inventory_path = "vagrant_hosts"
    ...
    # Update
#   springboot.vm.provision "update", type: "ansible" do |ansible|
